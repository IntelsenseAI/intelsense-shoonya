...
const SuperCheckerAudioTranscriptionLandingPage = () => {
    ...
    const ProjectDetails = useSelector(
        (state) => state.getProjectDetails?.data
    );
    ...
    useEffect(() => {
        const hasEmptyText = result?.some(
    ...
    const handleAutosave = async () => {
        ...
        if (result.length && taskDetails?.super_check_user === userData.id) {
            try {
                const obj = new SaveTranscriptAPI(
            ...
            if (userAnnotation) {
                ...
                if (reviewerAnnotations.length > 0) {
                    ...
                    if (correctAnnotation) {
                        ...
                        setreviewtext(
                            reviewNotesRef.current.getEditor().getText()
                        );
                        setsupercheckertext(
                            superCheckerNotesRef.current.getEditor().getText()
                        );
            ...
            <Grid container direction={"row"} className={classes.parentGrid}>
              ...
              /> */}
                                    ...
                                    renderComponent={(props) => (
                                        ...
                                        >
                                            <textarea
                                                ...
                                                style={{
                                                    fontSize:
                                                        stdTranscriptionSettings.fontSize,
                                                    height: "120px",
                                                }}
                                                {...props}
                                            />
                        ...
                        <div
                            className={classes.collapse}
                            style={{
                                display: advancedWaveformSettings
                                    ? "block"
                                    : "none",
                                marginTop: "15%",
                                overflow: "auto",
